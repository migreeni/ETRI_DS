...
valid_dates = set(pd.to_datetime(train_df['lifelog_date']).dt.date) | set(pd.to_datetime(submission_df['lifelog_date']).dt.date)
merge_df = merge_df[merge_df['lifelog_date'].isin(valid_dates)]

# 3. 하루 단위 그룹화 - 평균과 표준편차 계산
grouped_mean = (
...
grouped_std.columns = [f"{col}_std" for col in grouped_std.columns]

daily_df = pd.concat([grouped_mean, grouped_std], axis=1).reset_index()
...
submission_df.to_csv("submission_minseok.csv", index=False)
