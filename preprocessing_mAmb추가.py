...
print("Loading data...")
mUsageStats = pd.read_parquet('ch2025_data_items/ch2025_mUsageStats.parquet')
mActivity = pd.read_parquet('ch2025_data_items/ch2025_mActivity.parquet')
mBle = pd.read_parquet('ch2025_data_items/ch2025_mBle.parquet')
mWifi = pd.read_parquet('ch2025_data_items/ch2025_mWifi.parquet')
...
mAmbience_expanded['timestamp'] = mAmbience_expanded['timestamp'].dt.floor('10T')

prob_cols = [
    col for col in mAmbience_expanded.columns
    if col not in ['subject_id', 'timestamp']
]

prep_mAmbience = (
    mAmbience_expanded
    .groupby(['subject_id', 'timestamp'])[prob_cols]
    .mean()
    .reset_index()
)
